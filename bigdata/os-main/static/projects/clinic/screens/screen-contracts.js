(function (global) {
    ...
    try {
      ...
      if (json.success && json.results) {
        ...
        app.setState(function (prev) {
          ...
          // 5. Populate 'clinic_patients' (for patient creation modal)
          refData['clinic_patients'] = Object.assign({}, refData['clinic_patients'] || {}, {
            // Add any FK tables that clinic_patients references
            // (typically none, but keeping structure consistent)
          });
        ...
        handler: async function (_ev, ctx) {
          ...
          try {
            ...
            if (res.success) {
              ...
              pushNotification(ctx, 'success', lang === 'ar' ? 'تم حفظ الخدمة طبية بنجاح' : 'Contract saved successfully');
  ...
  // resolveSchemaColumns and ensureTableMeta removed - obsolete
    ...
    var modalContent = D.Div({ attrs: { class: 'space-y-6' } }, [
      ...
      renderTableSection(lang === 'ar' ? 'تفاصيل الخدمات' : 'Service Lines', null, D.Div({ attrs: { class: 'space-y-4' } }, [
        ...
        UC.Table({
          ...
          lang: lang,
          actions: (function () {
            // Show booking button for new contracts OR edit mode with future bookings without check-in
            var isNewContract = !editor.form || !editor.form.id;
            var isEditMode = !isNewContract && !readOnly;

            return [
              (allowLineEdit || isEditMode) ? { key: 'contracts:open-wizard', label: lang === 'ar' ? 'حجز' : 'Schedule', icon: '📅', variant: 'outline' } : null,
              allowLineEdit ? { key: 'contracts:line-edit', label: lang === 'ar' ? 'تعديل' : 'Edit', icon: '✏️', variant: 'outline' } : null,
              allowLineEdit ? { key: 'contracts:line-remove', label: lang === 'ar' ? 'حذف' : 'Remove', icon: '🗑️', variant: 'danger' } : null
            ].filter(Boolean);
          })()
        }),
  ...
  // renderModal, resolveSchemaColumns, and ensureTableMeta removed - obsolete
  ...
  function renderDashboardIframeModal(appState) {
    ...
    var iframeContent = D.Div({ attrs: { class: 'w-full h-[85vh] bg-white' } }, [
      D.Iframe({
        attrs: {
          src: modal.url,
  ...
  function resolveSchemaColumns(appState, tableName) {
    var state = appState.data.screens.contracts || {};
    var cache = state.columnMetaCache || {};
    return cache[tableName] || [];
  }

  function ensureTableMeta(app, tableName) {
    return new Promise(function (resolve) {
      var state = app.getState();
      var sc = state.data.screens.contracts || {};
      var cache = sc.columnMetaCache || {};
      if (cache[tableName]) {
        return resolve(cache[tableName]);
      }
      M.REST.repo(tableName).search({ lang: state.env.lang, limit: 1, withMeta: 1 }).then(function (res) {
        var meta = res.columnsMeta || [];
        app.setState(function (prev) {
          var scPrev = prev.data.screens.contracts || {};
          var cachePrev = Object.assign({}, scPrev.columnMetaCache || {});
          cachePrev[tableName] = meta;
          return Object.assign({}, prev, {
            data: Object.assign({}, prev.data, {
              screens: Object.assign({}, prev.data.screens, {
                contracts: Object.assign({}, scPrev, { columnMetaCache: cachePrev })
              })
            })
          });
        });
  ...
  function renderInfoModal(appState) {
    ...
    var content = D.Div({ attrs: { class: 'space-y-6' } }, [
      ...
      renderTableSection(lang === 'ar' ? 'سجل الحجز' : 'Bookings', null, UC.Table({
        columns: [
          {
            key: 'slot_label',
            label: lang === 'ar' ? 'الموعد' : 'Slot',
            render: function (row) {
              var label = row.slot_label || '';
              // Convert 24h times to AM/PM (e.g., "14:00" → "2:00 م")
              return D.Span({}, [label.replace(/(\d{1,2}):(\d{2})/g, function (match, h, m) {
                return toAmPm(h + ':' + m);
              })]);
            }
          },
        ...
        lang: lang,
        actions: [
          { key: 'contracts:edit-bookings', label: lang === 'ar' ? 'تعديل المواعيد' : 'Edit Bookings', icon: '📅', variant: 'outline' }
        ]
      })),
      D.Div({ attrs: { class: 'grid md:grid-cols-3 gap-3 text-sm' } }, [
        D.Div({ attrs: { class: 'rounded-lg border border-[var(--border)] p-3 flex items-center justify-between' } }, [
    ...
    return UI.Modal({
      open: true,
      title: lang === 'ar' ? 'تفاصيل الخدمة طبية' : 'Contract Details',
      size: 'lg',
  ...
  function renderScreen(appState) {
    ...
    return D.Div({ attrs: { class: 'space-y-4' } }, [
      D.Div({ attrs: { class: 'flex flex-wrap items-center justify-between gap-3' } }, [
        ...
        D.Div({ attrs: { class: 'flex flex-wrap items-center gap-2' } }, [
          UC.Button({ key: 'contracts:new', label: lang === 'ar' ? 'خدمة طبية جديدة' : 'New Medical Service', icon: '➕', variant: 'primary' })
        ])
      ]),
      ...
      renderDashboardIframeModal(appState),
      ...
      // Note: SchemaCrud global modals render automatically, no need for renderModal() here
    ]);
  }
  ...
  function renderPrintModal(appState) {
    ...
    if (print.loading || !print.details) {
      ...
      var metaSections = [
        [
          ...
          { labelEn: 'Contract Date', labelAr: 'تاريخ الخدمة طبية', value: contractDate }
        ],
        [
      ...
      content = D.Div({ attrs: { class: 'print-preview-container p-8 bg-white text-black space-y-8' } }, [
        D.Div({ attrs: { class: 'space-y-3 border-b pb-4' } }, [
          D.Div({ attrs: { class: 'flex flex-col md:flex-row md:items-end md:justify-between gap-2' } }, [
            ...
            D.Div({ attrs: { class: 'text-xs uppercase tracking-[0.2em] text-[var(--muted-foreground)]' } }, [lang === 'ar' ? 'رقم الخدمة طبية' : 'Contract #']),
          ...
          statusLabel ? D.Div({ attrs: { class: 'text-xs text-[var(--muted-foreground)] flex items-center gap-1' } }, [
            D.Span({}, [lang === 'ar' ? 'حالة الخدمة طبية' : 'Contract Status']),
    ...
    return UI.Modal({
      open: true,
      title: lang === 'ar' ? 'طباعة الخدمة طبية' : 'Print Contract',
      size: 'xl',
  ...
  function resolveDayLabel(day, lang) {
    if (!day) return '';
    if (lang === 'ar' && day.dayName) return day.dayName; // Trust backend label first

    var dayNum = Number(day.dayNum);
    var arDays = ['الأحد', 'الاثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة', 'السبت'];
    if (lang === 'ar') {
      return Number.isFinite(dayNum) ? arDays[dayNum] : (day.dayName || '');
    }
    return day.dayName || day.dayNameEn || '';
  }
  ...
  function normalizeGridSlot(slot, fallbackDate) {
    ...
    if (slot.slot_time_start) {
      return Object.assign({}, slot, {
        slot_status: slot.slot_status || slot.status || 'available',
        slot_date: slot.slot_date || fallbackDate,
        capacity: slot.capacity,
        booked_count: slot.booked_count || slot.bookedCount || slot.booked,
        remaining: slot.remaining
      });
    }

    var duration = Number(slot.duration || 15);
    var startTime = slot.time || slot.timeStart || '';
    var endTime = slot.timeEnd || '';
    ...
    return {
      ...
      slot_time_start: startTime,
      slot_time_end: endTime,
      slot_status: slot.slot_status || slot.status || 'available',
  ...
  function buildBlockFromSlotRecord(slot) {
    var normalized = normalizeGridSlot(slot, slot && (slot.slot_date || slot.date));
    if (!normalized || !normalized.id) return null;
    var start = normalized.slot_time_start || '';
    var end = normalized.slot_time_end || '';
    var label = (start ? start.slice(0, 5) : '') + (end ? (' - ' + end.slice(0, 5)) : '');
    return {
      id: normalized.id,
      blockId: String(normalized.id || '') + '|' + String(normalized.slot_date || '') + '|' + String(normalized.slot_time_start || ''),
      slot_date: normalized.slot_date,
      slot_time_start: normalized.slot_time_start,
      slot_time_end: normalized.slot_time_end,
      label: label,
      slots: [normalized],
      slot: normalized,
      status: normalized.slot_status
    };
  }

  function buildBlocksForDay(day, baseSlotMinutes, serviceMinutes) {
    var allSlots = [];
    (day.slots || []).forEach(function (bucket) {
      // New Structure: Bucket with stations
      if (bucket.stations && Array.isArray(bucket.stations)) {
        bucket.stations.forEach(function (s) {
          var status = String(s.status || s.slot_status || '').toLowerCase();
          // Include both available AND booked slots
          // Explicitly extract ID to avoid Object/Circular references
          var slotId = s.id || (s.slot && s.slot.id);
          // Construct clean slot object
          var slotObj = {
            id: slotId,
            slot_date: day.date,
            time: bucket.time || s.time,
            timeEnd: bucket.timeEnd || s.timeEnd,
            status: status,
            slot_status: status,
            station: s.station || (s.slot && s.slot.station),
            capacity: s.capacity || (s.slot && s.slot.capacity),
            booked_count: s.booked_count || s.bookedCount || (s.slot && (s.slot.booked_count || s.slot.bookedCount)),
            remaining: s.remaining || (s.slot && s.slot.remaining)
          };
          var norm = normalizeGridSlot(slotObj, day.date);
          if (norm && norm.id) allSlots.push(norm);
        });
      } else {
        // Old Structure - include all slots
        var nested = bucket.slot || bucket;
        var norm = normalizeGridSlot(nested, day.date);
        if (norm) allSlots.push(norm);
      }
    });

    // Bypass buildSlotBlocks which might be filtering 15m slots for 30m services
    // We assume backend provides valid start times.
    return allSlots.map(function (slot) {
      var startMins = timeToMinutes(slot.slot_time_start);
      var duration = Number(serviceMinutes || slot.duration || baseSlotMinutes || 15);
      var endMins = (startMins !== null) ? startMins + duration : null;
      var timeEnd = (endMins !== null) ? minutesToTime(endMins) : (slot.slot_time_end || slot.slot_time_start);

      var label = (slot.slot_time_start || '').slice(0, 5) + ' - ' + (timeEnd || '').slice(0, 5);

      return {
        id: slot.id,
        blockId: String(slot.id || '') + '|' + String(slot.slot_date || '') + '|' + String(slot.slot_time_start || ''),
  ...
  async function confirmContract(ctx) {
    ...
    if (!lines.length) {
      alert(lang === 'ar' ? 'أضف بنود الخدمة طبية أولاً' : 'Add contract lines first');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (response && (response.success || response.contractId)) {
        setLoading(false);
        alert(lang === 'ar' ? 'تم تأكيد الخدمة طبية بنجاح' : 'Contract confirmed');
        loadScreen(ctx);
        ctx.setState(function (prev) {
          var sc = prev.data.screens.contracts || {};
          return Object.assign({}, prev, {
      ...
      alert(lang === 'ar' ? 'فشل تأكيد الخدمة طبية: ' + msg : 'Failed to confirm contract: ' + msg);
    }
  }
  ...
  async function openContractEditor(ctx, contractId) {
    ...
    // Build existing bookings map from booking items (scheduled) so wizard shows saved bookings
    var requestToLine = {};
    (details.bookingRequests || []).forEach(function (req) {
      var reqId = getRecordId(req);
      var lineId = getRecordId(req.contract_line);
      if (reqId && lineId) requestToLine[reqId] = lineId;
    });
    var slotsIndex = details.slotsIndex || {};
    var lineBookings = {};
    (details.bookingItems || []).forEach(function (item) {
      var requestId = getRecordId(item.booking_request || item.request);
      var lineId = requestToLine[requestId];
      var slotId = getRecordId(item.slot);
      if (!lineId || !slotId || !slotsIndex[slotId]) return;
      var block = buildBlockFromSlotRecord(slotsIndex[slotId]);
      if (!block) return;
      if (!lineBookings[lineId]) lineBookings[lineId] = [];
      lineBookings[lineId].push(block);
    });
    editor.lineBookings = lineBookings;
    editor.selectedSlots = Object.keys(lineBookings).reduce(function (acc, key) {
      return acc.concat(lineBookings[key] || []);
    }, []);
  ...
  async function loadAvailabilityGrid(ctx, options) {
    ...
    var daysCount = Number((options && options.days_count) || wiz.days_count || 0);
    // Smart Days: only auto-calc when user didn't manually set it
    if (!wiz.days_count_manual) {
      var sessions = Number(wiz.sessions_count || 12);
      daysCount = Math.max(10, sessions * 5);
    }
    ...
    try {
      ...
      if (json && json.success) {
        ...
        var serviceMinutes = resolveActiveServiceMinutes(editor, services, packages, clinicType);
        // Don't filter out days - show all days with slots (available or booked)
        var rawDays = json.days || json.calendar || []; // Fix: Support both keys
        var days = rawDays.filter(function (day) {
        ...
        D.Div({ attrs: { class: 'grid md:grid-cols-2 gap-4' } }, [
          ...
          D.Div({ attrs: { class: 'md:col-span-2' } }, [
            ...
            D.Div({ attrs: { class: 'flex flex-wrap gap-4' } }, [
              ...
              return D.Label({ attrs: { class: 'flex items-center gap-2 cursor-pointer border border-[var(--border)] p-2 rounded hover:bg-[var(--accent)] ' + (isChecked ? 'bg-blue-500/10 border-blue-500/50' : '') } }, [
                D.Input({
      ...
      content = D.Div({ attrs: { class: 'space-y-4' } }, [
        ...
        D.Div({ attrs: { class: 'flex justify-between items-center mb-2' } }, [
          ...
          D.Div({ attrs: { class: 'flex items-center gap-2' } }, [
            ...
            wiz.selectedSlots && wiz.selectedSlots.length > 0 ? UC.Button({
              key: 'contracts:wiz:apply-pattern',
              label: lang === 'ar' ? 'تطبيق التكرار (Apply Series)' : 'Apply Series',
              icon: '🔁',
              variant: 'primary',
              size: 'sm'
            }) : null,
        ...
        (function () {
          ...
          gridDays.forEach(function (day) {
            (day.slots || []).forEach(function (slot) {
              ...
              var station = (nested && nested.station) || slot.station;
              if (station && station.room_code) {
                allStations[station.id || station.room_code] = station.room_code;
              }
            });
          });
          var stationKeys = Object.keys(allStations);
          if (stationKeys.length <= 1) return null; // Only show tabs if multiple stations

          var selectedStation = wiz.selectedStation || 'all';
          return D.Div({ attrs: { class: 'flex gap-2 mb-4 border-b pb-2' } }, [
            D.Button({
              attrs: {
                class: 'px-4 py-2 rounded-t text-sm font-medium transition-colors ' +
                  (selectedStation === 'all' ? 'bg-primary text-primary-foreground' : 'bg-[var(--muted)] text-[var(--muted-foreground)] hover:bg-[var(--accent)]'),
                gkey: 'contracts:wiz:select-station',
                'data-station': 'all'
              }
            }, [lang === 'ar' ? 'كل المحطات' : 'All Stations']),
            stationKeys.map(function (stationKey) {
              var isSelected = selectedStation === stationKey;
              return D.Button({
                attrs: {
                  class: 'px-4 py-2 rounded-t text-sm font-medium transition-colors ' +
                    (isSelected ? 'bg-primary text-primary-foreground' : 'bg-[var(--muted)] text-[var(--muted-foreground)] hover:bg-[var(--accent)]'),
                  gkey: 'contracts:wiz:select-station',
                  'data-station': stationKey
                }
              }, [allStations[stationKey] || stationKey]);
            })
          ]);
        })(),
        D.Div({ attrs: { class: 'flex flex-wrap gap-2 text-xs text-[var(--muted-foreground)]' } }, [
          D.Span({ attrs: { class: 'px-2 py-1 rounded-full bg-emerald-100 text-emerald-900 border border-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-100 dark:border-emerald-500/50' } }, [lang === 'ar' ? 'فارغ' : 'Empty']),
          D.Span({ attrs: { class: 'px-2 py-1 rounded-full bg-amber-100 text-amber-900 border border-amber-200 dark:bg-amber-900/40 dark:text-amber-100 dark:border-amber-500/50' } }, [lang === 'ar' ? 'مشغول جزئياً' : 'Partial']),
          D.Span({ attrs: { class: 'px-2 py-1 rounded-full bg-red-100 text-red-900 border border-red-200 dark:bg-red-900/40 dark:text-red-100 dark:border-red-500/50' } }, [lang === 'ar' ? 'مكتمل' : 'Full']),
          D.Span({ attrs: { class: 'px-2 py-1 rounded-full bg-[var(--muted)] text-[var(--muted-foreground)] border border-[var(--border)]' } }, [lang === 'ar' ? 'محجوب' : 'Blocked']),
          D.Span({ attrs: { class: 'px-2 py-1 rounded-full bg-blue-50 text-blue-800 border border-blue-200 dark:bg-blue-900/40 dark:text-blue-100 dark:border-blue-500/50' } }, [
              ...
              return D.Div({
                attrs: {
                  class: 'border rounded-lg p-3 ' +
                    (isClosed ? 'bg-[var(--muted)] opacity-60 decoration-dashed' :
                      hasSlots ? 'bg-[var(--card)] hover:shadow-md transition-shadow border-[var(--border)]' : 'bg-[var(--muted)]/50')
                }
              }, [
                // Day Header
                  ...
                  !hasBlocks ?
                    ...
                    D.Div({ attrs: { class: 'space-y-2' } }, [
                      D.Div({ attrs: { class: 'space-y-1' } }, blocks.map(function (block) {
                        ...
                        // Capacity + status handling
                        var slotStatus = String(slot && (slot.slot_status || slot.status) || '').toLowerCase();
                        var capacity = Math.max(1, Number(slot && slot.capacity || 1));
                        var bookedCount = Number(slot && (slot.booked_count || slot.bookedCount || slot.booked) || 0);
                        var remaining = (slot && typeof slot.remaining === 'number') ? slot.remaining : Math.max(0, capacity - bookedCount);
                        var isBlockedStatus = slotStatus === 'blocked' || (slot && slot.is_active === 0);
                        var isFull = remaining <= 0 || slotStatus === 'booked' || slotStatus === 'full';
                        var isPartial = !isFull && (remaining < capacity || slotStatus === 'partial');
                        var isBooked = isFull || isPartial;
                        ...
                        if (isBlockedStatus) {
                          computedClass = 'bg-[var(--muted)] text-[var(--muted-foreground)] border-[var(--border)] cursor-not-allowed opacity-70';
                          isDisabled = true;
                        } else if (isFull) {
                          computedClass = 'bg-red-600/20 text-red-700 dark:text-red-200 border-red-500/40 dark:bg-red-900/40 cursor-not-allowed';
                          isDisabled = !bookingMeta;
                        } else if (isPartial) {
                          computedClass = 'bg-amber-400/20 text-amber-800 dark:text-amber-200 border-amber-400/50 dark:bg-amber-900/40 cursor-pointer';
                        } else if (bookingMeta) {
                          computedClass = 'bg-orange-500/15 text-orange-600 dark:text-orange-200 border-orange-500/30 cursor-pointer';
                        } else if (isBlocked) {
                          computedClass = 'bg-[var(--muted)] text-[var(--muted-foreground)] border-[var(--border)] cursor-not-allowed';
                          isDisabled = true;
                        } else if (isSelected) {
                          computedClass = 'bg-primary text-primary-foreground border-primary shadow-sm';
                        } else {
                          // Available default (empty)
                          computedClass = 'bg-emerald-400/15 text-emerald-900 dark:text-emerald-200 border-emerald-300/50 dark:bg-emerald-900/30 hover:bg-emerald-400/25';
                        }

                        var showCapacity = capacity > 1 || (slot && typeof slot.remaining === 'number');
                        var occupancyLabel = isBlockedStatus ? (lang === 'ar' ? 'محجوب' : 'Blocked')
                          : isFull ? (lang === 'ar' ? 'مكتمل' : 'Full')
                            : showCapacity ? ((lang === 'ar' ? 'متبقي ' : 'Left ') + remaining + (lang === 'ar' ? ' من ' : ' / ') + capacity) : '';
                        var occupancyRatio = capacity > 0 ? Math.min(1, Math.max(0, (capacity - remaining) / capacity)) : 0;
                        ...
                        return D.Button({
                          ...
                          D.Div({ attrs: { class: 'flex items-center justify-between' } }, [
                            ...
                            D.Span({ attrs: { class: 'text-[10px] px-2 py-0.5 rounded-full bg-white/80 dark:bg-black/30' } }, [durationLabel])
                          ]),
                          showCapacity ? D.Div({ attrs: { class: 'mt-1 space-y-1' } }, [
                            D.Div({ attrs: { class: 'text-[10px] font-semibold' } }, [occupancyLabel]),
                            D.Div({ attrs: { class: 'h-1.5 rounded-full bg-black/10 dark:bg-white/10 overflow-hidden' } }, [
                              D.Div({
                                attrs: {
                                  class: (isFull ? 'bg-red-600' : (isPartial ? 'bg-amber-500' : 'bg-emerald-500')) + ' h-full',
                      ...
                      // Booked slots are now included in the main blocks list above
    ...
    var footer = isViewer ? D.Div({ attrs: { class: 'flex justify-end gap-2 pt-4 border-t mt-4' } }, [
      ...
      wiz.step === 1
        ...
        : D.Div({ attrs: { class: 'flex gap-2' } }, [
          ...
          UC.Button({
            key: 'contracts:wiz:confirm',
            label: lang === 'ar' ? 'تأكيد الحجز' : 'Confirm Booking',
            icon: '✅',
            variant: 'primary',
            disabled: (wiz.selectedSlots || []).length < (wiz.sessions_count || 1)
          })
        ])
    ]);
  ...
  global.ClinicScreens.contracts = {
    ...
    render: function (app) {
      ...
      // SchemaCrud will render global modals automatically - no custom renderModal needed
      return D.Div({}, [base, cal, wiz].filter(Boolean));
    },
    orders: {
      ...
      'contracts:patient-search': {
        ...
        handler: async function (ev, ctx) {
          ...
          ctx.setState(function (prev) {
            ...
            search.query = value;
            search.open = !!trimValue;
            search.loading = !!trimValue;
            if (!trimValue) search.results = [];
          ...
          if (ctx._patientSearchTimer) clearTimeout(ctx._patientSearchTimer);
          if (!trimValue) return;

          ctx._patientSearchTimer = setTimeout(async function () {
            try {
              var lang = ctx.getState().env.lang;
              var res = await M.REST.repo('clinic_patients').search({ lang: lang, q: trimValue, limit: 25 });
              var rows = res.data || res || [];
              ctx.setState(function (prev) {
                var sc = prev.data.screens.contracts || {};
                var editor = Object.assign({}, sc.editor || {});
                var search = Object.assign({}, editor.patientSearch || {});
                // Only update if query still matches what we searched (simple check: if loading was cancelled? no, just update)
                // Better: Check if trimValue is still relevant? 
                // With debounce, we assume this is the latest.
      ...
      'contracts:patient-advanced-search': {
        ...
        handler: async function (ev, ctx) {
          ...
          ctx.setState(function (prev) {
            ...
            modal.query = value;
            modal.loading = !!trimValue;
            if (!trimValue) modal.results = [];
          ...
          if (ctx._advSearchTimer) clearTimeout(ctx._advSearchTimer);
          if (!trimValue) return;

          ctx._advSearchTimer = setTimeout(async function () {
            try {
              var lang = ctx.getState().env.lang;
              var res = await M.REST.repo('clinic_patients').search({ lang: lang, q: trimValue, limit: 50 });
              var rows = res.data || res || [];
              ctx.setState(function (prev) {
                var sc = prev.data.screens.contracts || {};
                var editor = Object.assign({}, sc.editor || {});
      ...
      'contracts:patient-modal-open': {
        ...
        handler: function (_ev, ctx) {
          var state = ctx.getState();
          var lang = state.env.lang;
          var branchId = (state.env && state.env.branchId) || 'pt';

          // Open dashboard in iframe with activeTable=clinic_patients and hidefrm=true
          var dashboardUrl = '/projects/clinic/index.html?brname=' + branchId + '&activeTable=clinic_patients&hidefrm=true';
                ...
                screens: Object.assign({}, prev.data.screens, {
                  contracts: Object.assign({}, sc, {
                    dashboardIframeModal: {
                      open: true,
                      url: dashboardUrl,
                      title: lang === 'ar' ? 'إدارة المرضى' : 'Patient Management'
                    }
                  })
                })
              })
      ...
      'contracts:close-dashboard-iframe': {
        on: ['click'],
        gkeys: ['contracts:close-dashboard-iframe'],
        handler: function (_ev, ctx) {
          ctx.setState(function (prev) {
            var sc = prev.data.screens.contracts || {};
            return Object.assign({}, prev, {
              data: Object.assign({}, prev.data, {
                screens: Object.assign({}, prev.data.screens, {
                  contracts: Object.assign({}, sc, {
                    dashboardIframeModal: { open: false }
                  })
                })
              })
            });
      ...
      'contracts:patient-add-fk': {
        ...
        handler: function (_ev, ctx) {
          console.log('[contracts:patient-add-fk] Not implemented yet');
        }
      },
      ...
      'crud:generic:tab': {
        ...
        handler: function (ev, ctx) {
          var target = ev.target.closest('[data-tab]');
          if (!target) return;
      ...
      'crud:generic:update-translation': {
        ...
        handler: function (ev, ctx) {
          ...
          if (!fieldName || !langCode) return;

          if (ctx.orders && ctx.orders.updateContractsModal) {
            return ctx.orders.updateContractsModal(ctx, function (modal) {
              var next = Object.assign({}, modal);
              next.translations = Object.assign({}, next.translations || {});
              next.translations[langCode] = Object.assign({}, next.translations[langCode] || {});
              next.translations[langCode][fieldName] = value;
              return next;
            });
          }
        }
      ...
      'contracts:modal-save': {
        on: ['click'],
        gkeys: ['contracts:modal-save', 'crud:generic:save'],
        handler: async function (_ev, ctx) {
          var state = ctx.getState();
          var sc = state.data.screens.contracts || {};
          var modal = sc.modal || {};
          if (!modal.table || !modal.form) return;

          // Set loading
          if (ctx.orders && ctx.orders.updateContractsModal) {
            ctx.orders.updateContractsModal(ctx, function (m) {
              return Object.assign({}, m, { loading: true, errors: {} });
            });
          }

          try {
            var repo = M.REST.repo(modal.table);
            var payload = { record: modal.form };

            // Add translations if any
            if (modal.translations && modal.translationFields && modal.translationFields.length) {
              var upserts = [];
              Object.keys(modal.translations).forEach(function (lang) {
                var data = modal.translations[lang];
                var hasContent = false;
                Object.keys(data).forEach(function (k) {
                  if (data[k]) hasContent = true;
                });
                if (hasContent) {
                  upserts.push({ langCode: lang, data: data });
                }
              });
              payload.translations = { upserts: upserts, removals: [] };
            }

            var result = await repo.save(payload);

            // Close modal
            if (ctx.orders && ctx.orders.updateContractsModal) {
              ctx.orders.updateContractsModal(ctx, function (m) {
                return Object.assign({}, m, { open: false, loading: false });
              });
            }

            // Refresh reference data
            await loadContractsReferenceData(ctx);

            console.log('[contracts:modal-save] Patient saved:', result);
          } catch (err) {
            console.error('[contracts:modal-save] Error:', err);
            if (ctx.orders && ctx.orders.updateContractsModal) {
              ctx.orders.updateContractsModal(ctx, function (m) {
                return Object.assign({}, m, { loading: false, errors: { _global: err.message || 'Save failed' } });
              });
            }
          }
        }
          ...
          ctx.setState(function (prev) {
            return Object.assign({}, prev, {
              data: Object.assign({}, prev.data, {
                modal: {
                  open: true,
                  table: 'clinic_contracts_lines',
                  form: {},
                  mode: 'create',
                  context: 'contracts:lines'
                }
              })
            });
          });
          ...
          ctx.setState(function (prev) {
            return Object.assign({}, prev, {
              data: Object.assign({}, prev.data, {
                modal: {
                  open: true,
                  table: 'clinic_contract_schedule_preferences',
                  form: form,
                  mode: 'create',
                  context: 'contracts:schedule'
                }
              })
            });
          });
      ...
      // Note: Modal save is now handled by crud:generic:save from SchemaCrud
      'crud:generic:save': {
        on: ['click'],
        gkeys: ['crud:generic:save'],
        handler: async function (_ev, ctx) {
          var state = ctx.getState();
          var modal = state.data.modal || {};

          // Check if this is a contracts-related modal
          if (!modal.context || !modal.context.startsWith('contracts:')) {
            return; // Let other handlers handle it
          }

          var contextType = modal.context.replace('contracts:', '');
          var sc = state.data.screens.contracts || {};

          // Apply the save to the appropriate contracts editor list
          ctx.setState(function (prev) {
            var scp = prev.data.screens.contracts || {};
            ...
            if (contextType === 'lines') {
              var lines = (editor.lines || []).slice();
              var idx = lines.findIndex(function (row) { return String(row.id) === String(form.id); });
              if (idx >= 0) lines[idx] = form; else lines.push(form);
              editor.lines = lines;
              editor.form.total_amount = lines.reduce(function (sum, row) { return sum + Number(row.price_total || 0); }, 0);
            } else if (contextType === 'schedule') {
              var schedules = (editor.schedule || []).slice();
              var sIdx = schedules.findIndex(function (row) { return String(row.id) === String(form.id); });
              if (sIdx >= 0) schedules[sIdx] = form; else schedules.push(form);
              editor.schedule = schedules;
            } else if (contextType === 'payments') {
              var payments = (editor.payments || []).slice();
              var pIdx = payments.findIndex(function (row) { return String(row.id) === String(form.id); });
              if (pIdx >= 0) payments[pIdx] = form; else payments.push(form);
              editor.payments = payments;
            ...
            return Object.assign({}, prev, {
              data: Object.assign({}, prev.data, {
                screens: Object.assign({}, prev.data.screens, {
                  contracts: Object.assign({}, scp, { editor: editor })
                }),
                modal: { open: false }
              })
            });
      ...
      // Note: Modal close is now handled by crud:generic:close from SchemaCrud
      'contracts:confirm': {
        on: ['click'],
        gkeys: ['contracts:confirm'],
        handler: function (_ev, ctx) {
        ...
        handler: async function (ev, ctx) {
          ...
          ctx.setState(function (prev) {
            ...
            if (!form.supervising_doctor && !form.executing_doctor) {
              alert(state.env.lang === 'ar' ? 'يجب اختيار الطبيب أولاً في بيانات الخدمة طبية' : 'Please select a doctor in contract details first');
              return prev;
            }
            ...
            wiz.days_count = Math.max(10, sessionsCount * 5);
            wiz.days_count_manual = false;
      ...
      'contracts:open-availability-grid': {
        ...
        handler: async function (_ev, ctx) {
          ctx.setState(function (prev) {
            ...
            wiz.doctor = form.supervising_doctor || form.executing_doctor;
            wiz.start_date = form.start_date || new Date().toISOString().slice(0, 10);
            var defaultSessions = Number(wiz.sessions_count || 12);
            wiz.days_count = Math.max(10, defaultSessions * 5);
            wiz.days_count_manual = false;
            editor.bookingWizard = wiz;
      ...
      'contracts:wiz:update': {
        ...
        handler: function (ev, ctx) {
          ...
          ctx.setState(function (prev) {
            ...
            wiz[field] = value;
            if (field === 'days_count') {
              wiz.days_count_manual = true;
            }
            if (field === 'sessions_count' && !wiz.days_count_manual) {
              var sessions = Number(value || 1);
              wiz.days_count = Math.max(10, sessions * 5);
            }
            editor.bookingWizard = wiz;
            return Object.assign({}, prev, {
              data: Object.assign({}, prev.data, {
      ...
      'contracts:wiz:analyze': {
        ...
        handler: async function (_ev, ctx) {
          ...
          ctx.setState(function (prev) {
            ...
            var editor = Object.assign({}, sc.editor || {});
            var wiz = Object.assign({}, editor.bookingWizard || {});
            var sessions = Number(wiz.sessions_count || 1);
            // calc: minimum 10, or sessions * 5 (e.g. 12 -> 60)
            var smartDays = Math.max(10, sessions * 5);
            wiz.days_count = smartDays;
            editor.bookingWizard = wiz;
            return Object.assign({}, prev, { data: Object.assign({}, prev.data, { screens: Object.assign({}, prev.data.screens, { contracts: Object.assign({}, sc, { editor: editor }) }) }) });
          });
          await loadAvailabilityGrid(ctx);
        }
      },
      ...
      'contracts:wiz:apply-pattern': {
        on: ['click'],
        gkeys: ['contracts:wiz:apply-pattern'],
        handler: function (ev, ctx) {
          ctx.setState(function (prev) {
            var state = prev;
            var sc = state.data.screens.contracts || {};
            var editor = Object.assign({}, sc.editor || {});
            var wiz = Object.assign({}, editor.bookingWizard || {});
            var selectedSlots = (wiz.selectedSlots || []).slice();
            var sessionsCount = Number(wiz.sessions_count || 1);
            var gridDays = wiz.gridData && wiz.gridData.days ? wiz.gridData.days : [];

            if (!selectedSlots.length) {
              alert(state.env.lang === 'ar' ? 'يرجى اختيار موعد واحد على الأقل كنموذج' : 'Please select at least one slot as a pattern');
              return prev;
            }
            if (selectedSlots.length >= sessionsCount) {
              alert(state.env.lang === 'ar' ? 'اكتمل العدد المطلوب بالفعل' : 'Required sessions count already met');
              return prev;
            }

            // Extract patterns from current selection
            var patterns = selectedSlots.map(function (s) {
              // Get date info
              var dateStr = s.slot_date || (s.slots && s.slots[0] && s.slots[0].slot_date);
              if (!dateStr) return null;
              var d = new Date(dateStr).getDay();

              // Get time - check multiple possible fields
              var t = s.slot_time_start || s.time || (s.slots && s.slots[0] && s.slots[0].slot_time_start);
              if (!t) return null;

              return { dayOfWeek: d, time: t.slice(0, 5) };
            }).filter(Boolean);

            if (!patterns.length) {
              alert(state.env.lang === 'ar' ? 'لا يمكن تحديد نمط من المواعيد المحددة' : 'Could not determine pattern from selected slots');
              return prev;
            }

            // Get blocked blocks to avoid conflicts
            var blockedBlocks = collectBookedBlocks(editor.lineBookings || {}, wiz.contract_line);

            // Iterate future days
            var addedCount = 0;
            var needed = sessionsCount - selectedSlots.length;
            ...
            gridDays.forEach(function (day) {
              ...
              // Check if this day matches any pattern
              var pattern = patterns.find(function (p) { return p.dayOfWeek === dayOfWeek; });
              if (!pattern) return;

              // Find matching block by time
              var match = day.blocks.find(function (block) {
                // Get block start time
                var blockTime = null;
                if (block.slots && block.slots[0]) {
                  blockTime = block.slots[0].slot_time_start || block.slots[0].time;
                } else if (block.slot) {
                  blockTime = block.slot.slot_time_start || block.slot.time;
                }
                if (!blockTime) return false;
                return blockTime.slice(0, 5) === pattern.time;
              });

              if (!match) return;

              // Check if block is available (not in blockedBlocks)
              var isBlocked = blockedBlocks.some(function (reserved) {
                return blockOverlaps(match, reserved);
              });
              if (isBlocked) return;

              // Check if slot inside block is available
              var slotAvailable = false;
              if (match.slots && match.slots.length > 0) {
                slotAvailable = match.slots.some(function (slot) {
                  var status = String(slot.slot_status || slot.status || '').toLowerCase();
                  return status === 'available';
                });
              } else if (match.slot) {
                var status = String(match.slot.slot_status || match.slot.status || '').toLowerCase();
              ...
              var exists = selectedSlots.some(function (sel) {
                return sel.blockId === match.blockId;
              });
            ...
            wiz.selectedSlots = selectedSlots;
            editor.bookingWizard = wiz;
            return Object.assign({}, prev, { data: Object.assign({}, prev.data, { screens: Object.assign({}, prev.data.screens, { contracts: Object.assign({}, sc, { editor: editor }) }) }) });
          });
        }
      ...
      'contracts:wiz:confirm': {
        ...
        handler: async function (_ev, ctx) {
          ...
          if (toBook.length < (wiz.sessions_count || 1)) {
            alert(state.env.lang === 'ar' ? 'يجب اختيار جميع الجلسات المطلوبة (' + wiz.sessions_count + ')' : 'Please select all required sessions (' + wiz.sessions_count + ')');
            return;
          }
  ...
  function renderManagerModal(ctx) {
    ...
    if (global.ClinicSchemaCrud && global.ClinicSchemaCrud.renderManager) {
      ...
      return UI.Modal({
        ...
        hideFooter: true,
        content: D.Div({ attrs: { class: 'h-[80vh] flex flex-col' } }, [content])
      });
    }
  }
                ...
                D.Td({ attrs: { class: 'p-2 border' } }, [
                  day.hasSlots
                    ? D.Div({ attrs: { class: 'flex flex-wrap gap-2' } }, day.slots.map(function (slot) {
                      ...
                      var isPartial = slot.status === 'partial';

                      if (isBooked || isBlocked) {
                        return D.Button({
                          attrs: {
                            class: 'px-2 py-1 rounded bg-gray-100 text-gray-400 text-sm cursor-not-allowed border border-gray-200 flex flex-col items-center justify-center opacity-70',
                            disabled: true,
                            title: isBooked ? 'Full' : 'Blocked'
                          }
                        }, [
                          D.Span({}, ['🔒 ' + slot.time]),
                          // Show capacity if full
                          isBooked && slot.capacity > 1 ? D.Span({ attrs: { class: 'text-[10px] text-red-400' } }, ['Full (' + slot.capacity + ')']) : null
                        ]);
                      }
                      ...
                      var baseClass = isSelected
                        ...
                        : isPartial
                          ? 'bg-teal-50 text-teal-800 border-teal-200 hover:bg-teal-100' // Distinct style for Partial
                          : 'bg-white text-gray-700 border-gray-300 hover:bg-blue-50 hover:border-blue-300';

                      return D.Button({
                        attrs: {
                          class: 'px-3 py-1 rounded text-sm cursor-pointer border transition-colors duration-200 flex flex-col items-center justify-center min-w-[80px] ' + baseClass,
                        ...
                        D.Span({ attrs: { class: 'font-semibold' } }, [(isSelected ? '✔ ' : '') + slot.time]),
                        // Show remaining seats for partial/available slots if capacity > 1
                        (slot.capacity && slot.capacity > 1) ?
                          D.Span({ attrs: { class: 'text-[10px] opacity-80' } }, [
                            (lang === 'ar' ? 'متبقي: ' : 'Left: ') + (slot.remaining !== undefined ? slot.remaining : slot.capacity)
                          ]) : null
                      ]);
                    }))
