(function (global) {
    ...
    if (!M || !M.DSL || !M.app || !UC || !M.REST) {
        console.error('[Clinic Dashboard] Required libraries missing (Mishkah, DSL, App, UniversalComp, REST)');
        return;
    }
    ...
    var STORAGE_KEYS = {
        theme: 'clinic_crud_theme',
        lang: 'clinic_crud_lang',
        tablePrefs: 'clinic_crud_table_prefs_v2'
    };
    ...
    try {
        demoAuthKey = global.__clinicAuthKey || (global.localStorage ? global.localStorage.getItem('clinic:auth_api_key') : null) || null;
    } catch (_err) {
        demoAuthKey = null;
    }
    ...
    // Read URL parameters for iframe mode
    var urlParams = {};
    try {
        var searchParams = new URLSearchParams(window.location.search);
        urlParams.activeTable = searchParams.get('activeTable') || null;
        urlParams.hidefrm = searchParams.get('hidefrm') === 'true';
    } catch (e) {
        urlParams.activeTable = null;
        urlParams.hidefrm = false;
    }
    console.log('[Dashboard Init] urlParams:', urlParams);
    console.log('urlParams.activeTable', urlParams.activeTable);
    ...
    var initialState = {
        env: {
            ...
            hideFormSidebar: urlParams.hidefrm
        },
        ...
        data: {
            ...
            tableSort: {},
            activeGroupByTable: {},
            activeTable: urlParams.activeTable,
            records: [],
            total: 0,
            page: 1,
            loading: !!urlParams.activeTable,
            loadingRecord: false,
            searchTerm: '',
            error: null,
            languages: [],
            ...
            columnsMeta: null,
            companyInfo: null,
            activeScreen: urlParams.activeTable ? 'settings' : 'home',
            screens: {
                home: { loading: false, stats: {}, recentBookings: [], chartData: null },
                profiles: { loading: false, page: 1, limit: 20, search: '', list: [], total: 0, selected: null, selectedId: null },
                contracts: { loading: false, page: 1, limit: 20, search: '', list: [], total: 0, selected: null, selectedId: null, lines: [] },
                bookings: { loading: false, page: 1, limit: 20, search: '', list: [], total: 0, selected: null, selectedId: null },
                progress: { loading: false, page: 1, limit: 20, search: '', list: [], total: 0, selected: null, selectedId: null, steps: [] },
                finance: { loading: false, invoices: [], payments: [] }
            },
            sidebarSearch: '',
            selectedRecord: null,
            editRecord: null,
    ...
    // Parse URL parameters for embed mode and table navigation
    try {
        var urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get('embedMode') === 'true' || urlParams.get('hidefrn') === 'true') {
            initialState.data.embedMode = true;
            initialState.data.sidebarCollapsed = true;
        }
        var initialTable = urlParams.get('tbn');
        if (initialTable) {
            initialState.data.activeTable = initialTable;
            initialState.data.activeScreen = 'settings';
        }
    } catch (e) { console.error('Error parsing URL params', e); }

    console.log('initialState.data.activeTable  1', initialState.data.activeTable);
    // ============================================================================
    // SCHEMA HELPERS
    // ============================================================================
    ...
    function getFkDefs(schemaInfo, tableName) {
        ...
        if (!tableDef || !tableDef.fkReferences) {
            console.log('[FK Debug] No FK references found in schema for table:', tableName);
            return [];
        }
        ...
        console.log('[FK Debug] Loaded FK definitions from schema for', tableName, ':', fkDefs);
        return fkDefs;
    }
    ...
    function collectFkDefs(schemaInfo, tableName, records) {
        ...
        console.log('[FK Debug] collectFkDefs for table:', tableName, '- found', defs.length, 'FK columns from schema');
        return defs;
    }
    ...
    async function ensureFkObjects(app, tableName, records) {
        ...
        async function hydrateTarget(targetTable, ids) {
            ...
            console.log('[FK Debug] hydrateTarget: Fetching', ids.length, 'records from table:', targetTable);
            var repo = M.REST.repo(targetTable);
            var cacheForTable = fkCache[targetTable] || {};
            var fetches = ids.map(async function (id) {
                if (cacheForTable[id]) {
                    console.log('[FK Debug] → Using cached entry for ID:', id);
                    return;
                }
                try {
                    console.log('[FK Debug] → Fetching record:', targetTable + '/' + id, 'with lang:', lang);
                    var res = await repo.get(id, { lang: lang });
                    console.log('[FK Debug] → API Response for', id, ':', res);
                    var rec = res.record || res;
                    var displayName = displayNameForRecord(rec, targetTable, schemaInfo, lang);
                    console.log('[FK Debug] → Resolved display name for', id, ':', displayName);
            ...
            console.log('[FK Debug] → Updated cache for', targetTable, ':', cacheForTable);
        }
    ...
    var orders = {
        ...
        'auth:update-user-insert': {
            on: ['input', 'change'],
            gkeys: ['auth:update-user-insert'],
            handler: function (ev, ctx) {
                var target = ev && (ev.target || ev.currentTarget) ? (ev.target || ev.currentTarget) : null;
                if (!target) return;
                var name = target.getAttribute('name') || target.getAttribute('data-field');
        ...
        'crud:save-record': {
            ...
            handler: async function (_ev, ctx) {
                ...
                try {
                    ...
                    var recordId = payload && (payload.id || payload.Id || payload.uuid);
                    if (!recordId) {
                        var editRecord = state.data.editRecord || {};
                        recordId = editRecord.id || editRecord.Id || editRecord.uuid;
                    }
                    var response;
                    if (recordId) {
        ...
        'crud:create': {
            ...
            handler: async function (_ev, ctx) {
                ...
                var fields = ensureTranslationFields(state.data.translationFields, {}).filter(function (f) { return f !== 'display_name'; });
        ...
        'crud:open-fk-quick-add': {
            ...
            handler: async function (ev, ctx) {
                ...
                try {
                    ...
                    var columnsMeta = normalizeColumnsMeta(res.columnsMeta || []);
                    var translationFields = columnsMeta.filter(function (col) { return col && col.source === 'lang'; }).map(function (col) { return col.name; }).filter(function (f) { return f !== 'display_name'; });
        ...
        'crud:save-fk-quick-add': {
            ...
            handler: async function (_ev, ctx) {
                ...
                try {
                    ...
                    var langPayload = buildLangPayload(translationsPayload);
                    var payload = Object.assign({}, recordPayload, langPayload ? { _lang: langPayload } : null);
                    var res = await repo.create(payload, { lang: state.env.lang });
    ...
    function classifyModule(tableName) {
        ...
        if (!name.startsWith('clinic_')) return 'operations';

        var settingsKeys = [
            'clinic_companies', 'clinic_branches', 'clinic_clinics', 'clinic_departments', 'clinic_rooms', 'clinic_devices',
            'clinic_items', 'clinic_services', 'clinic_service_domains', 'clinic_specialties', 'clinic_patients',
            'clinic_types', 'clinic_stations', 'clinic_tariffs', 'clinic_packages'
        ];
        var operationsHints = ['booking', 'contract', 'visit', 'ticket', 'slot', 'invoice', 'payment', 'progress'];
        var reportHints = ['report', 'view', 'history', 'trail'];
        var logHints = ['log', 'audit'];

        if (settingsKeys.includes(name)) return 'settings';
        if (operationsHints.some(function (hint) { return name.indexOf(hint) !== -1; })) return 'operations';
        if (reportHints.some(function (hint) { return name.indexOf(hint) !== -1; })) return 'reports';
        if (logHints.some(function (hint) { return name.indexOf(hint) !== -1; })) return 'logs';
        return 'operations';
    }
    ...
    function buildColumnsForTable(db, displayRows) {
        ...
        if (!displayRows.length && columnsMeta.length) {
            columnsMeta.forEach(function (col) {
                if (col && col.name) availableKeys.add(col.name);
            });
        }
        availableKeys.add('__display');
        var groups = getTableGroups(tableName, data.schemaInfo, env.lang);
    ...
    function getAuthContextFromSession() {
        try {
            var raw = global.localStorage ? global.localStorage.getItem('mishkah_user') : null;
            if (!raw) return { companyId: null, branchId: null, userId: null };
            var parsed = JSON.parse(raw);
            return {
                companyId: parsed && (parsed.compid || parsed.company_id || parsed.companyId) || null,
                branchId: parsed && (parsed.branch_id || parsed.branchId || parsed.branch) || null,
                userId: parsed && (parsed.userID || parsed.userId || parsed.user_id || parsed.id) || null
            };
        } catch (_err) {
            return { companyId: null, branchId: null, userId: null };
        }
    }
    ...
    function isHiddenColumn(key) {
        ...
        if (['id', 'company_id', 'company', 'branch_id', 'branch', 'user_insert', 'display_name', 'translations', 'i18n', '__translation'].includes(lower)) {
            return true;
        }
    ...
    function buildSavePayload(dataState) {
        ...
        if (!baselineRecord && dataState.columnsMeta) {
            var tableName = dataState.recordTable || dataState.activeTable;
            var sequenceFields = getSequenceFieldSet(tableName, dataState.columnsMeta, dataState.schemaInfo);
    ...
    async function loadContextInfo(app) {
        ...
        var sessionContext = getAuthContextFromSession();
        if (!cookieContext.companyId && sessionContext.companyId) {
            setCookie('company_id', sessionContext.companyId, 30);
        }
        if (!cookieContext.branchId && sessionContext.branchId) {
            setCookie('branch_id', sessionContext.branchId, 30);
        }
        if (!cookieContext.userId && sessionContext.userId) {
            setCookie('user_insert', sessionContext.userId, 30);
        }
        cookieContext = getAuthContextFromCookies();
        var effectiveContext = {
            companyId: cookieContext.companyId || sessionContext.companyId || null,
            branchId: cookieContext.branchId || sessionContext.branchId || null,
            userId: cookieContext.userId || sessionContext.userId || null
        };
        var missing = [];
        if (!effectiveContext.companyId) missing.push('company_id');
        if (!effectiveContext.branchId) missing.push('branch_id');
        if (!effectiveContext.userId) missing.push('user_insert');

        if (missing.length) {
            app.setState(function (prev) {
                var nextForm = Object.assign({}, prev.data.authForm || {});
                if (effectiveContext.companyId) nextForm.company_id = effectiveContext.companyId;
                if (effectiveContext.branchId) nextForm.branch_id = effectiveContext.branchId;
                if (effectiveContext.userId) nextForm.user_insert = effectiveContext.userId;
                return Object.assign({}, prev, {
                    data: Object.assign({}, prev.data, {
                        authRequired: true,
        ...
        var company = await fetchById('companies', effectiveContext.companyId);
        var branch = await fetchById('branches', effectiveContext.branchId);
        var user = await fetchById('users', effectiveContext.userId);
    ...
    function resolveBranchFromUrl() {
        try {
            ...
            return currentUrl.searchParams.get('brname') || currentUrl.searchParams.get('branch') || null;
        } catch (_err) {
            return null;
        }
    }

    function resolveBranchFromLocalStorage() {
        try {
            var raw = localStorage.getItem('mishkah_user');
            if (!raw) return null;
            var parsed = JSON.parse(raw);
    ...
    function getBranchId(state) {
        var ctxBranch = state && state.data && state.data.defaultContext && state.data.defaultContext.branch
            ? state.data.defaultContext.branch
            : null;
        var candidate = ctxBranch && (ctxBranch.code || ctxBranch.branch || ctxBranch.branch_code || ctxBranch.id || ctxBranch.name);
        if (candidate && !isLikelyUuid(String(candidate)) && !/\s/.test(String(candidate))) return String(candidate);
        return resolveBranchFromLocalStorage() || readCookie('branch') || readCookie('brname') || resolveBranchFromUrl() || global.MISHKAH_BRANCH || global.BRANCH_ID || candidate || null;
    }
    ...
    function getSequenceColumns(columnsMeta, tableName, schemaInfo) {
        var names = getSequenceFieldSet(tableName, columnsMeta, schemaInfo);
        return Array.from(names).map(function (name) { return { name: name }; });
    }
    ...
    async function fetchSequencePreview(branchId, moduleId, tableName, fieldName) {
        ...
        var safeModule = moduleId || (global.MODULE_ID || 'clinic');
    ...
    async function refreshSequenceHints(app, tableName) {
        ...
        var columnsMeta = normalizeColumnsMeta(state.data.columnsMeta || []);
        var sequenceColumns = getSequenceColumns(columnsMeta, tableName, state.data.schemaInfo);
        if (!sequenceColumns.length) return;
        var branchId = getBranchId(state);
        var moduleId = global.MODULE_ID || 'clinic';
    ...
    function getDynamicTabs(appState) {
        var lang = appState.env && appState.env.lang || 'en';

        // Base Home Tab
        var tabs = [{ id: 'home', label: lang === 'ar' ? 'الرئيسية' : 'Home', icon: '🏠' }];
        ...
        var screenLabels = {
            profiles: { ar: 'بروفايل العملاء', en: 'Profiles' },
            bookings: { ar: 'الحجوزات', en: 'Bookings' },
            contracts: { ar: 'الخدمات الطبية', en: 'Medical Services' },
            progress: { ar: 'التقدم', en: 'Progress' },
            finance: { ar: 'المالية', en: 'Finance' }
        };

        if (global.ClinicScreens) {
            Object.keys(global.ClinicScreens).forEach(function (key) {
                if (key === 'home') return;
                var labels = screenLabels[key] || {};
                tabs.push({
                    id: key,
                    label: labels[appState.env.lang] || labels.ar || labels.en || key,
                    icon: screenIcons[key] || '🗂️'
                });
            });
        }

        // Add settings at the end
        tabs.push({ id: 'settings', label: lang === 'ar' ? 'الإعدادات' : 'Settings', icon: '🧰' });
    ...
    function loadActiveScreen(app, screenId) {
        ...
        if (!global.ClinicScreens || !global.ClinicScreens[screenId]) return;
        var screen = global.ClinicScreens[screenId];
    ...
    function renderActiveScreen(appState) {
        ...
        if (!global.ClinicScreens || !global.ClinicScreens[screenId]) {
        ...
        return global.ClinicScreens[screenId].render(appState);
    }
        ...
        try {
            ...
            var uiTables = rawTables
                .filter(function (t) {
                    ...
                    return name && name.indexOf('clinic_') === 0 && !name.endsWith('_lang');
                })
                .map(function (t) {
            ...
            uiTables.forEach(function (t) {
                ...
                if (name.includes('patient')) newModuleId = 'operations_patients';
                else if (name.includes('booking')) newModuleId = 'operations_appointments';
                else if (name.includes('contract')) newModuleId = 'operations_appointments';
                else if (name.includes('invoice') || name.includes('payment')) newModuleId = 'financial_transactions';
                else if (name.includes('tariff') || name.includes('package') || name.includes('pricing')) newModuleId = 'financial_pricing';
                else if (name.includes('doctor') || name.includes('staff')) newModuleId = 'healthcare_staff';
                else if (name.includes('clinic') || name.includes('department') || name.includes('room')) newModuleId = 'healthcare_clinical';
                else if (name.includes('company') || name.includes('branch')) newModuleId = 'core_structure';
                else if (name.includes('device') || name.includes('item') || name.includes('service')) newModuleId = 'core_resources';
                else if (name.includes('audit') || name.includes('log')) newModuleId = 'system';
                else if (name.includes('ui_') || name.includes('label')) newModuleId = 'system';
                // Type-based fallback
                else if (t.type === 'operations') newModuleId = 'operations';
                else if (t.type === 'reports') newModuleId = 'financial';
                else if (t.type === 'logs') newModuleId = 'system';
                else if (t.type === 'settings') newModuleId = 'system';
            ...
            app.setState(function (prev) {
                return Object.assign({}, prev, {
                    data: Object.assign({}, prev.data, {
                        ...
                        activeTable: prev.data.activeTable || null,
                        records: [],
                        selectedRecord: null,
                        editRecord: null
                    })
    ...
    function renderRecordEditor(db) {
        ...
        var columnsMeta = normalizeColumnsMeta(data.columnsMeta || []);
        var sequenceFields = getSequenceFieldSet(activeTable, columnsMeta, data.schemaInfo);
        ...
        function renderFieldControl(col) {
            ...
            var isSequenceField = sequenceFields.has(field) || !!col.sequence || (col.default_expr && String(col.default_expr).startsWith('sequence:'));
            var isReadonly = col.readonly === true || col.is_read_only === true || field === 'id' || isSequenceField;
                ...
                return D.Div({ attrs: { class: 'flex flex-col gap-1.5', key: 'field-' + field } }, [
                    ...
                    D.Select({
                        attrs: {
                            ...
                            disabled: isReadonly ? 'disabled' : undefined,
            ...
            var baseAttrs = { name: field, gkey: 'crud:update-field', disabled: isReadonly ? 'disabled' : undefined, readOnly: isReadonly ? 'readonly' : undefined };
            if (heur.type === 'date') {
                return D.Div({ attrs: { class: 'flex flex-col gap-1.5', key: 'field-' + field } }, [
        ...
        function renderGroupTab(groupId) {
            ...
            if (!fields.length) {
                var fallbackFields = Object.keys(record || {}).filter(function (key) {
                    ...
                    if (isHiddenColumn(key)) return false;
        ...
        function renderFkQuickModal() {
            ...
            function renderQuickAddFields() {
                ...
                var sequenceFields = getSequenceFieldSet(quick.target, columnsMeta, data.schemaInfo);
                var editableColumns = columnsMeta.filter(function (col) {
                ...
                function renderQuickField(col) {
                    ...
                    var baseAttrs = {
                        'data-field': field,
                        name: field,
                        gkey: 'crud:update-fk-quick-field',
                        disabled: isReadonly ? 'disabled' : undefined,
    ...
    function renderSidebar(db) {
        ...
        function renderModule(module, depth) {
            ...
            if (!matches(label) && !hasTables && !visibleChildren.length) return null;
        ...
        return D.Aside({
            ...
            D.Div({
                ...
                D.Div({ attrs: { class: 'flex items-center gap-3 flex-1' } }, [
                    ...
                    !isCollapsed ? D.Div({}, [
                        D.Div({ attrs: { class: 'font-bold text-lg text-white' } }, [companyName || (env.lang === 'ar' ? 'عيادة برو' : 'Clinic Pro')]),
                        D.Div({ attrs: { class: 'text-white/80 text-xs' } }, [env.lang === 'ar' ? 'إدارة متكاملة' : 'Unified Management'])
                    ]) : null
                ].filter(Boolean)),
        ...
        var actions = [
            {
                screen: 'profiles',
                icon: '🧍',
                title: { ar: 'بروفايلات العملاء', en: 'Profiles' },
                desc: { ar: 'بيانات العملاء والمرفقات والبحث المتقدم', en: 'Profiles, attachments, and advanced search' }
            },
            {
                screen: 'contracts',
                icon: '📑',
                title: { ar: 'الخدمات الطبية', en: 'Contracts' },
                desc: { ar: 'تفاصيل التعاقدات والجلسات والمدفوعات', en: 'Contracts, sessions, and payments' }
            },
            {
                screen: 'bookings',
                icon: '📅',
                title: { ar: 'الحجوزات', en: 'Bookings' },
                desc: { ar: 'حجز وإعادة جدولة ومتابعة الوصول', en: 'Schedule, reschedule, and check-ins' }
            },
            {
                screen: 'progress',
                icon: '⏱️',
                title: { ar: 'التنفيذ', en: 'Progress' },
                desc: { ar: 'إدارة خطوات التنفيذ والمتابعة', en: 'Execution steps and progress tracking' }
            },
            {
                screen: 'finance',
                icon: '💰',
                title: { ar: 'المالية', en: 'Finance' },
                desc: { ar: 'الفواتير والتحصيل والتدفق النقدي', en: 'Invoices, collections, cashflow' }
            }
        ];

        if (!actions.length) return null;

        return D.Div({ attrs: { class: 'grid md:grid-cols-3 gap-3' } }, actions.map(function (action) {
            return D.Button({
                attrs: {
                    type: 'button',
                ...
                D.Div({ attrs: { class: 'text-2xl mb-2' } }, [action.icon]),
                D.Div({ attrs: { class: 'font-semibold text-lg mb-1' } }, [action.title[env.lang] || action.title.en]),
                D.P({ attrs: { class: 'text-sm text-[var(--muted-foreground)] leading-relaxed' } }, [action.desc[env.lang] || action.desc.en])
            ]);
        }));
    }
    ...
    function renderBody(db) {
        ...
        var activeLabel = getActiveTableLabel(data.tables, data.activeTable, env.lang) || 'Clinic';
        ...
        var sidebar = !data.embedMode ? renderSidebar(db) : null;
            ...
            var gateContent = D.Div({ attrs: { class: 'rounded-3xl border border-[var(--border)] bg-[var(--card)] p-8 shadow-xl space-y-6 max-w-xl w-full' } }, [
                ...
                isLoading ? D.Div({ attrs: { class: 'text-sm text-[var(--muted-foreground)]' } }, [env.lang === 'ar' ? 'جارٍ التحميل...' : 'Loading...']) : D.Div({ attrs: { class: 'grid gap-4' } }, [
                    ...
                    D.Div({ attrs: { class: 'space-y-1' } }, [
                        D.Label({ attrs: { class: 'text-xs font-semibold text-[var(--muted-foreground)]' } }, [env.lang === 'ar' ? 'المستخدم' : 'User']),
                        D.Input({
                            attrs: {
                                type: 'text',
                                name: 'user_insert',
                                gkey: 'auth:update-user-insert',
                                value: form.user_insert || '',
                                class: 'w-full h-11 rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 text-sm focus:ring-2 focus:ring-[var(--primary)]'
                            }
                        })
                    ]),
        ...
        // Hide Quick Actions when ANY table is open (activeTable exists)
        var showQuickActions = !data.activeTable && !env.hideFormSidebar && !data.embedMode;
        console.log('[Dashboard Render] activeTable:', data.activeTable, 'hideFormSidebar:', env.hideFormSidebar, 'embedMode:', data.embedMode, 'showQuickActions:', showQuickActions);
        if (data.activeScreen && data.activeScreen !== 'settings' && !data.activeTable) {
            content = D.Div({ attrs: { class: 'space-y-4' } }, [
                showQuickActions ? renderQuickActions(db) : null,
                customScreen
            ].filter(Boolean));
        ...
        } else if (data.loading || (data.activeTable && !data.schemaInfo)) {
            ...
            if (!tableRows.length) {
                tableRows = [D.Tr({}, [
                    D.Td({
                        attrs: {
                            class: 'px-3 py-6 text-center text-sm text-[var(--muted-foreground)]',
                            colspan: columns.length
                        }
                    }, [env.lang === 'ar' ? 'لا توجد بيانات بعد. يمكنك إضافة أول سجل.' : 'No data yet. You can add the first record.'])
                ])];
            }
            ...
            content = D.Div({ attrs: { class: 'space-y-5' } }, [
                // Removed: renderQuickActions - should NOT show when table is open
        ...
        // If hideFormSidebar is true (iframe mode), return BARE DOMAIN
        // We want ONLY the content (Table/Grid) and Modals. No Sidebar, No Header, No Tabs, No Breadcrumbs.
        if (env.hideFormSidebar) {
            return D.Div({ attrs: { class: 'min-h-screen bg-[var(--background)] p-4 flex flex-col' } }, [
                // Add a small header for the iframe to allow closing if needed, or just rely on parent modal
                // content is the Table Grid or Active Screen
                content
            ].concat(overlays).filter(Boolean));
        }
    ...
    function collectScreenOrders() {
        var output = {};
        if (!global.ClinicScreens) return output;
        Object.keys(global.ClinicScreens).forEach(function (key) {
            var screen = global.ClinicScreens[key];
    ...
    (function checkAuth() {
        ...
        try {
            ...
            userData = global.localStorage ? global.localStorage.getItem('mishkah_user') : null;
        } catch (_err) {
            token = null;
            userData = null;
        }

        // If no token and no user session, redirect
    ...
    loadTables(app);
    // Load Initial Data
    console.log('initialState.data.activeTable', initialState.data.activeTable)

    // Auto-load table if specified in params
    if (initialState.data.activeTable) {
        // Trigger select table logic manually since loadActiveScreen might not handle table loading deep logic
        // Initial cache buster log
        console.log('[Dashboard] v1.1.0 Loaded');

        // Robust Loader: Wait for Schema to be ready before checking activeTable data
        function ensureSchemaAndLoadTable(targetTable, attempts) {
            attempts = attempts || 0;
            if (attempts > 20) {
                console.warn('[Dashboard] Schema load timeout, forcing loadTableData anyway.');
                loadTableData(app, targetTable);
                return;
            }

            var state = app.getState();
            // Check if schemaInfo is populated (it is null initially)
            if (state.data && state.data.schemaInfo && state.data.schemaInfo.tableMap) {
                console.log('[Dashboard] Schema ready. Loading active table:', targetTable);
                // Ensure activeTable is set (in case loadTables wiped it)
                app.setState(function (prev) {
                    return Object.assign({}, prev, { data: Object.assign({}, prev.data, { activeTable: targetTable }) });
                });
                loadTableData(app, targetTable);
            } else {
                console.log('[Dashboard] Waiting for schema... attempt', attempts);
                setTimeout(function () {
                    ensureSchemaAndLoadTable(targetTable, attempts + 1);
                }, 200);
            }
        }

        ensureSchemaAndLoadTable(initialState.data.activeTable, 0);
        // Also trigger modal open if requested? The user said "iframe ... hides everything ... except crud".
        // If we want to open the CREATE modal automatically, we should check another param e.g. mode=create
        try {
            var up = new URLSearchParams(window.location.search);
            // If we are in embed mode, we likely want to force create mode immediately?
            // The user says "same screen we click in dashboard", meaning the LIST view + New Button?
            // Or directly the modal? "opening empty..." implies they want the MODAL.
            // If we want the MODAL to open automatically, we need to set showFormModal: true
            // But we need metadata first. loadTableData does that.
            // Let's add a small delay or check to open modal.
            if (up.get('embedMode') === 'true') {
                // For now just load the table. The user can click "New". 
                // actually the user said: "when we click add new client... iframe opens... same screen we click in dashboard"
                // This implies the iframe should show the standard CRUD interface.
                // If they want the MODAL to be open by default, we can add that logic.
                // Let's stick to showing the CRUD list first, as requested "same screen".
                // Wait, user said "opens completely empty... preventing interaction". 
                // The GOAL is to have a working modal.
                // If I open the iframe, I see the list. Then I click "New".
                // That's acceptable.
            }
        } catch (e) { }
    } else {
