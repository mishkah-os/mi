(function (global) {
  ...
  const fallbackShifts = extractDatasetArray(initialPayload, 'pos_shift');
  ...
  const initialShifts = initialRecords('pos_shift', fallbackShifts);
  ...
  // Filter orders by payment status type
  const filterOrdersByType = (orders = [], orderType = 'closed') => {
    if (orderType === 'open') {
      // Open: not finalized OR not paid
      return orders.filter(order => {
        const statusId = order.statusId || order.status_id || '';
        const paymentState = order.paymentStateId || order.payment_state_id || '';

        return statusId !== 'finalized' || paymentState !== 'paid';
      });
    } else {
      // Closed: finalized AND paid
      return orders.filter(order => {
        const statusId = order.statusId || order.status_id || '';
        const paymentState = order.paymentStateId || order.payment_state_id || '';
  ...
  // Filter orders by shift and payment status
  const filterOrdersByShift = (orders = [], payments = [], selectedShiftId = null, orderType = 'closed') => {
    if (!selectedShiftId || selectedShiftId === '') {
      // No shift filter - return all orders filtered by type only
      return filterOrdersByType(orders, orderType);
    }

    if (orderType === 'open') {
      // Open orders: filter by order_header.shiftId
      const filtered = orders.filter(order => {
        const shiftId = order.shiftId || order.shift_id;
        return shiftId === selectedShiftId;
      });
      return filterOrdersByType(filtered, 'open');
    } else {
      // Closed orders: filter by order_payment.shiftId
      const paymentsInShift = payments.filter(payment => {
        const shiftId = payment.shiftId || payment.shift_id;
        return shiftId === selectedShiftId;
      });

      const orderIdsInShift = new Set(
        paymentsInShift.map(p => p.orderId || p.order_id).filter(Boolean)
      );

      const filtered = orders.filter(order => {
        const orderId = order.id || order.order_id;
        return orderIdsInShift.has(orderId);
      });

      return filterOrdersByType(filtered, 'closed');
    }
  };
  ...
  const computeShiftSummaries = ({ shifts, orders, payments, paymentMethods }) => {
    const summaries = [];
    ...
    for (const shift of ensureArray(shifts)) {
      ...
      // Filter orders by this shift from order_header
      const shiftOrders = ensureArray(orders).filter(order => {
        const orderShiftId = order.shiftId || order.shift_id;
        return orderShiftId === shiftId;
      });

      // Filter payments by this shift
      const shiftPayments = ensureArray(payments).filter(payment => {
        const paymentShiftId = payment.shiftId || payment.shift_id;
        return paymentShiftId === shiftId;
      });

      // Separate completed (paid) vs open (unpaid) orders
      const completedOrders = shiftOrders.filter(order => {
        const statusId = order.statusId || order.status_id || '';
        const paymentState = order.paymentStateId || order.payment_state_id || '';
        return statusId === 'finalized' && paymentState === 'paid';
      });

      const openOrders = shiftOrders.filter(order => {
        const statusId = order.statusId || order.status_id || '';
        const paymentState = order.paymentStateId || order.payment_state_id || '';
        return statusId !== 'finalized' || paymentState !== 'paid';
      });
      ...
      const completedTotal = completedOrders.reduce((sum, order) => {
        return sum + ensureNumber(order.totalDue || order.total_due || order.totals?.due || 0);
      }, 0);

      const completedPaid = completedOrders.reduce((sum, order) => {
        return sum + ensureNumber(order.totalPaid || order.total_paid || order.totals?.paid || 0);
      }, 0);

      // Calculate totals for open orders
      const openTotal = openOrders.reduce((sum, order) => {
        return sum + ensureNumber(order.totalDue || order.total_due || order.totals?.due || 0);
      }, 0);

      const openPaid = openOrders.reduce((sum, order) => {
        return sum + ensureNumber(order.totalPaid || order.total_paid || order.totals?.paid || 0);
      }, 0);

      // Payment breakdown by method
      const paymentBreakdown = computePaymentBreakdown(shiftPayments, completedOrders, paymentMethods);

      // Total from payments
      const totalPayments = shiftPayments.reduce((sum, payment) => {
      ...
      summaries.push({
        shiftId,
        shift,
        userName: shift.metadata?.userName || shift.cashierName || shift.cashier_name || 'Unknown',
        openedAt: shift.openedAt || shift.opened_at,
        closedAt: shift.closedAt || shift.closed_at,
        isClosed: shift.isClosed || shift.status === 'closed',
        ...
        openOrders: {
          ...
          total: openTotal,
          paid: openPaid,
          outstanding: Math.max(openTotal - openPaid, 0)
        },
        paymentBreakdown,
        totalPayments,
  ...
  const buildState = ({ orders, payments, lines, shifts, summary, selectedShiftId, selectedOrderType = 'closed', newLookups, lang = activeLang, theme = activeTheme }) => {
    const filteredOrders = filterOrdersByShift(orders, payments, selectedShiftId, selectedOrderType);
    const recentOrders = filteredOrders.slice(0, 10);

    const shiftSummaries = computeShiftSummaries({
      shifts,
      orders,
      payments,
    ...
    const filteredSummary = computeFinancialSummary({
      orders: filteredOrders,
      payments: selectedShiftId
        ? payments.filter(p => (p.shiftId || p.shift_id) === selectedShiftId)
        : payments,
      shifts
    });

    // Add Total Row to Shift Summaries
    if (shiftSummaries.length > 0) {
      const totalSummary = shiftSummaries.reduce((acc, curr) => {
        return {
          completedOrders: {
            count: acc.completedOrders.count + curr.completedOrders.count,
            total: acc.completedOrders.total + curr.completedOrders.total,
            paid: acc.completedOrders.paid + curr.completedOrders.paid
          },
          openOrders: {
            count: acc.openOrders.count + curr.openOrders.count,
            total: acc.openOrders.total + curr.openOrders.total,
            paid: acc.openOrders.paid + curr.openOrders.paid,
            outstanding: acc.openOrders.outstanding + (curr.openOrders.outstanding || 0)
          },
          totalPayments: acc.totalPayments + curr.totalPayments,
          paymentBreakdown: [...acc.paymentBreakdown, ...curr.paymentBreakdown] // Will need aggregation
        };
      }, {
        completedOrders: { count: 0, total: 0, paid: 0 },
        openOrders: { count: 0, total: 0, paid: 0, outstanding: 0 },
        totalPayments: 0,
        paymentBreakdown: []
      });

      // Aggregate payment breakdown for total
      const aggPayments = {};
      totalSummary.paymentBreakdown.forEach(p => {
        const id = p.id || p.methodId; // Ensure ID exists
        if (id) {
          if (!aggPayments[id]) {
            aggPayments[id] = { ...p, amount: 0 };
          }
          aggPayments[id].amount += p.amount;
        }
      });
      totalSummary.paymentBreakdown = Object.values(aggPayments);

      shiftSummaries.push({
        shiftId: 'TOTAL',
        isTotal: true,
        userName: 'TOTAL', // Localization handled in UI component based on isTotal flag
        completedOrders: totalSummary.completedOrders,
        openOrders: totalSummary.openOrders,
        paymentBreakdown: totalSummary.paymentBreakdown,
        totalPayments: totalSummary.totalPayments
      });
    }
    ...
    return {
      ...
      data: {
        ...
        shifts,
        shiftSummaries,
        summary: filteredSummary // Use filtered summary for dashboard
      },
      ui: {
        selectedShiftId,
        selectedOrderType,
  ...
  const updateData = () => {
    ...
    const shifts = listRecords('pos_shift');
    console.log('[Finance Dashboard] updateData called:', { ordersCount: orders.length, paymentsCount: payments.length, shiftsCount: shifts.length, shifts });
    ...
    const currentShiftId = state?.ui?.selectedShiftId || selectedShiftId;
    const currentOrderType = state?.ui?.selectedOrderType || 'closed';

    app.setState(buildState({
      orders: normalizedOrders,
      payments,
      lines,
      shifts,
      ...
      lang: state?.data?.lang || activeLang,
      theme: state?.env?.theme || activeTheme
    }));
  };
  ...
  const handleShiftSelect = (event) => {
    const shiftId = event.target?.dataset?.shiftId || event.currentTarget?.dataset?.shiftId;
    if (!shiftId) return;
    selectedShiftId = selectedShiftId === shiftId ? null : shiftId;
    updateData();
  };

  const handleApproveShift = async (event) => {
    const shiftId = event.target?.dataset?.shiftId || event.currentTarget?.dataset?.shiftId;
    if (!shiftId || !db) return;

    try {
      const shifts = listRecords('pos_shift');
      const shift = shifts.find(s => (s.shift_id || s.id) === shiftId);
      if (!shift) return;

      const updatedShift = { ...shift, status: 'approved' };
      await db.update(getTableAlias('pos_shift'), updatedShift);

      setUiState({
        closingStatus: 'success',
        closingMessage: activeLang === 'ar' ? 'تم اعتماد الوردية بنجاح' : 'Shift approved successfully'
      });

      updateData();
    } catch (error) {
      console.error('[POS Finance] Failed to approve shift', error);
      setUiState({
        closingStatus: 'error',
        closingMessage: activeLang === 'ar' ? 'فشل اعتماد الوردية' : 'Failed to approve shift'
      });
    }
  };
  ...
  const handleCloseDay = async () => {
    const state = app.getState ? app.getState() : app.state;
    let ordersData = listRecords('orders');
    let paymentsData = listRecords('payments');
    let linesData = listRecords('lines');
    const shiftsData = listRecords('pos_shift');

    if (selectedShiftId) {
      ordersData = ordersData.filter(o => (o.shiftId || o.shift_id) === selectedShiftId);
      paymentsData = paymentsData.filter(p => (p.shiftId || p.shift_id) === selectedShiftId);
      const orderIds = new Set(ordersData.map(o => (o.id || o.order_id)));
      linesData = linesData.filter(l => orderIds.has(l.orderId || l.order_id));
    }
    ...
    try {
      ...
      let fullobject = {
        ...
        payments: paymentsData,
        lines: linesData,
        shifts: selectedShiftId ? shiftsData.filter(s => (s.shift_id || s.id) === selectedShiftId) : shiftsData,
        state,
        payload: dataClose
      };
  ...
  app.setOrders({
    ...
    'finance:shift:select': {
      on: ['click'],
      gkeys: ['finance:shift:select'],
      handler: handleShiftSelect
    },
    'finance:shift:select-change': {
      on: ['change'],
      gkeys: ['finance:shift:select-change'],
      handler: (event, ctx) => {
        const select = event?.target;
        if (!select) return;
        const shiftId = select.value || null;

        console.log('[POS Finance] Shift selection changed:', shiftId);

        const state = ctx.getState();
        const orders = state.data.allOrders || [];
        const payments = state.data.payments || [];
        const orderType = state.ui.selectedOrderType || 'closed';

        ctx.setState(buildState({
          orders,
          payments: state.data.payments,
          lines: state.data.orderLines,
    ...
    'finance:ordertype:select': {
      on: ['click'],
      gkeys: ['finance:ordertype:select'],
      handler: (event, ctx) => {
        const button = event?.target?.closest('[data-type]');
        if (!button) return;
        const orderType = button.getAttribute('data-type') || 'closed';

        console.log('[POS Finance] Order type changed:', orderType);

        const state = ctx.getState();
        const orders = state.data.allOrders || [];
        const shiftId = state.ui.selectedShiftId;
    ...
    'finance:shift:approve': {
      on: ['click'],
      gkeys: ['finance:shift:approve'],
      handler: handleApproveShift
    }
  });
